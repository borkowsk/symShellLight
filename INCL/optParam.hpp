...
///     W klasach potomnych obsługiwane są typy standardowe numeryczne
///     oraz inne typy mające implementacje << >> na strumienie i operator porównania <=
///     Obsługiwany jest też typ "const char*", ale na poziomie wskaźników,
///     czyli wskaźnik do zawartości linii komend może zostać przypisany na zmienną
///     typu "const char*"
...
class MAYBE_UNUSED OptionalParameter:public OptionalParameterBase
{
  ...
  public:
    ...
    /// Metoda konwersji. \details Muszą być dostarczone różne implementacje w zależności od typu T
...
class MAYBE_UNUSED OptEnumParametr:public OptionalParameter<T>
{
 protected:
    ...
    MAYBE_UNUSED
    void HelpPrn(ostream& o);     ///< Podstawienie dostarczone przez klasę bazową (???).

 public:
      /** Konstruktor. */ MAYBE_UNUSED
    ...
    /// Metoda konwersji. \details Musi być implementacja zależna od typu `EnNames` i `EnVals`.
...
/// Implementacja ogólna sprawdzania, czy parametr jest w zakresie <LBound...HBound>.
template<class T>
